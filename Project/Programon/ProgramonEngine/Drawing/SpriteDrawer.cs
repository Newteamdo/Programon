...
namespace ProgramonEngine
{
     public class SpriteDrawer
     {
          ...
          public void Draw(Camera camera, Player player, float deltaTime, IEnumerable<Node> addedNodes = null, IEnumerable<Actor> actors = null)
          {
               DrawBackground(camera);

               if (actors != null)
                    DrawActors(actors, camera, deltaTime);

               DrawPlayer(player, camera, deltaTime);
          ...
          private void DrawPlayer(Player player, Camera cam, float deltaTime)
          {
               if (player.IsWalking)
               {
                    Sprite frame;
                    switch (player.Direction)
                    {
                         ...
                         case Direction.RIGHT:
                              frame = player.Animations[AnimationTypes.WalkingLeft].NextFrame(deltaTime);
                              break;
                         case Direction.UP:
                              frame = player.Animations[AnimationTypes.WalkingUp].NextFrame(deltaTime);
                              break;
                         case Direction.DOWN:
                              frame = player.Animations[AnimationTypes.WalkingDown].NextFrame(deltaTime);
                              break;
                         default:
          ...
          private void DrawActors(IEnumerable<Actor> actors, Camera cam, float deltaTime)
          {
               for (int i = 0; i < actors.Count(); i++)
               {
                    ...
                    if (cur.IsWalking)
                    {
                         ...
                         switch (cur.Direction)
                         {
                              case Direction.LEFT:
                              case Direction.RIGHT:
                                   frame = cur.Animations[AnimationTypes.WalkingLeft].NextFrame(deltaTime);
                                   break;
                              case Direction.UP:
                                   frame = cur.Animations[AnimationTypes.WalkingUp].NextFrame(deltaTime);
                                   break;
                              case Direction.DOWN:
                                   frame = cur.Animations[AnimationTypes.WalkingDown].NextFrame(deltaTime);
                                   break;
                              default:
                                   goto case Direction.LEFT;
                         }
